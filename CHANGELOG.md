...
- **Errors are recorded in the changelog** - BMZ already kept the last 50 errors with their stack, but reading them needed a developer console, which is not available in the Android app. Errors now also appear in the bookmark changelog with the message, where it came from and the first line of the stack. They carry no Restore button, since there is nothing to restore, and a repeating error is recorded once every 30 seconds so a failing loop cannot flood the list.
