...
- **Connecting a device no longer creates duplicate bookmarks** - A bookmark held on both sides with a small difference, such as a trailing space, a renamed title or another folder, was copied in a second time.
...
- **The Bookmark Changelog is now the Event Log** - It no longer records only bookmark changes, so the old name had stopped being accurate. The three versions had also drifted apart and called it three different things: "Bookmark Changelog", "Bookmark Changes History" and "View Bookmark Changelog". All of them now read Event Log.
- **BMZ can now tell you about updates** - When something changes that is worth knowing about, a short message appears once as a toast the next time you open BMZ, and is kept in the Event Log so it is not lost if you missed it. Nothing is sent to you and nothing is tracked: BMZ reads one small file from the BMZ website on open, remembers which messages it has already shown, and stays silent if that file cannot be reached.
- **Errors are recorded in the Event Log** - BMZ already kept the last 50 errors with their stack, but reading them needed a developer console, which is not available in the Android app. Errors now appear in the Event Log with the message, where it came from and the first line of the stack. They carry no Restore button, since there is nothing to restore, and a repeating error is recorded once every 30 seconds so a failing loop cannot flood the list.

**Bug Fixes:**
- **The Copy button in the token viewer no longer runs off the edge** - The token field refused to shrink below the width of the token itself, which pushed the last button out of a narrow panel. The field now takes its own row and the buttons sit below it.
...
- **Setting up sync now asks how you want it done** - Four choices instead of a list of snippets. BMZ can create a private repository for you, use an empty one you made yourself, walk you through making one step by step, or connect to a repository that already holds your bookmarks. That last one reads what is already there and merges, rather than writing over it, which is what a second device needs.
- **Entering your token now takes you straight to setup** - It used to reopen the sync settings and leave you to find your own way onto a store. A device with nothing connected now goes to the setup screen instead.
- **Sync now tells you when it has stopped** - A store that has run out of room refuses every write and explains nothing. BMZ now recognises that specific failure, says so in plain terms, confirms your bookmarks are safe, and offers to move them. Previously syncing just stopped.
- **Change Repository** - Snippet users could always switch snippets. Repository users can now switch repositories from Cloud Sync Options, without disconnecting and re-entering your token.
- **The two overwrite buttons show which way your bookmarks move** - "Overwrite Cloud with Local" and "Overwrite Local with Cloud" differ only in word order, which is easy to misread when both are destructive. Each now carries an arrow, up for this device writing to the cloud and down for the cloud writing to this device.
- **Wording no longer says "Snippet" everywhere** - The settings, buttons, messages and logs now say cloud, because where your bookmarks live is a detail that can change. The snippet-specific screens keep their wording, since that is still what they do.
- **Pick your repository from a list instead of pasting an address** - BMZ already holds your GitLab token, so it now reads the repositories on your account and offers them in a dropdown on every screen that asks for one. Only repositories you can actually write to are listed, most recently active first, so one you just made is at the top. The address field is still there for anything the list cannot show.
- **Pointing at an existing repository checks what is in it first** - "Use an empty repository I already made" writes this device's bookmarks into whatever you choose, and never checked that it was empty. Typing a full address made a wrong choice unlikely; a dropdown of every repository makes it a slip. If the one you pick already holds bookmarks, BMZ says so, names the join option as the thing you probably meant, and only continues if you confirm. If it holds someone's actual project instead, BMZ names a couple of the files, explains that nothing there will be deleted but that it would commit to that repository on every sync, and asks. A repository holding only a README counts as empty, since that is what BMZ creates and what the instructions tell you to make.
- **The setup options are in a more useful order** - Connecting to a repository that already has your bookmarks is now first, because it is the right answer for every device except the one you set up first. Creating a new repository moved down, since on a second device that is the choice that splits your bookmarks across two stores. Change Repository and the migration screen also gained a Back button.

**Bug Fixes:**
- **Migrating a second device now offers to join the repository the first one made** - The move-to-a-repository screen showed only three choices and hid the fourth, on the reasoning that you are moving your own bookmarks rather than joining someone else's repository. That is only true of the first device you migrate. Every device after it is moving to a repository that already exists and already holds your bookmarks, and joining is the only correct answer for them, so the one option they needed was the one being hidden. All four choices now appear.
- **A failed sync no longer pretends it is waiting for you** - A sync that broke and a sync that stopped to ask your permission set the same flag, so a failure raised the card reading "Sync was paused to protect your data". Nothing had paused and nothing was held, so its Review changes button opened a dialog that found empty lists and closed again without drawing anything. The button looked broken because there was genuinely nothing to show. A failure now has its own card, in red, saying what actually went wrong.
- **Review changes always answers you** - When there is nothing waiting, it now says so and clears the card, instead of returning silently.
- **Background syncing now recognises a full store** - The sidebar could spot the specific error GitLab returns for a snippet that has run out of room, but the sidebar is not where unattended syncing happens. The background page read the status, threw away the message, retried twice more, gave up and raised a misleading card. It now names the failure, stops retrying something that can never succeed, and offers to move your bookmarks.
- **The sync settings offer a way in when nothing is connected** - Setup opens on its own right after you enter your token, but dismissing it left the settings dialog with only Disconnect and Cancel and no route to a store at all. It now shows a Set Up Bookmark Sync button whenever this device is not connected to anything.
- **Connecting for the first time no longer offers to delete your own bookmarks** - When a bookmark is on this device but not in the cloud, BMZ decides what to do by checking whether it watched you add it. If it did, the bookmark is sent up. If it has no record, it assumes another device deleted it and asks your permission first. A device connecting for the very first time has no records at all, so its own bookmarks looked like things to delete. Everything already on the device is now treated as yours at the moment you first connect.
- **A bookmark whose name differs by an invisible space is no longer treated as renamed** - Browsers happily store a title with a trailing space, and exporting and re-importing strips it. The two copies then disagreed over a character nobody can see, producing a request to approve renaming something to the name it already had. Names are now compared ignoring spaces at either end, and neither copy is rewritten.
